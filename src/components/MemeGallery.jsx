...
const MemeGallery = () => {
  ...
  const handleBid = async (id) => {
    try {
      await bidOnMeme(id, 10); // You can make this dynamic later
    } catch (err) {
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 px-4">
      {memes.map((meme) => (
        ...
        >
          ...
          <p className="text-sm text-yellow-300">Caption: {meme.caption || 'Not generated yet'}</p>
          <p className="text-sm text-green-400">Vibe: {meme.vibe || 'Not generated yet'}</p>
